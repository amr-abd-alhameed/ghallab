// import "./section.css";import { Link } from "react-router-dom";// import Button from "../buttons-component/solidbutton";// import { scrollToTop } from "../../constants/scrollToTop";// import { animationVariants } from "../../constants/animationVariants";// import { motion } from "framer-motion";// // import bg from "../../assets/images/maqbara/w27.jpg";
// const Section = ({ url, title, desc, href, attachment }) => {
//   return (
//     <div
//       className={`h-screen flex bg-top relative bg-no-repeat bg-cover  items-center`}
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        variants={animationVariants.zoomOut}
        viewport={{ once: true, amount: 0.2 }}
        className="flex flex-col max-w-lg mr-28 max-lg:mr-10 max-sm:mx-auto max-sm:px-5  text-white items-start gap-6 relative  w-fit max-sm:w-full max-sm:items-center max-sm:justify-center max-sm:text-center "
      >
        <h1 className="text-6xl max-sm:text-[42px] font-semibold">{title}</h1>
        <p className="text-2xl  max-sm:w-full">{desc}</p>
        <Link onClick={scrollToTop} to={href}>
